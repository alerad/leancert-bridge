...
Lean Runtime 4 environment libraries contain source-free check capsules, not
the sources and build inputs required to launch an interactive Bridge. Full
environments for kernel replay and registered enclosure profiles are therefore
materialized from the exact package reference instead of being published by
this workflow.

Manual workflow dispatch can publish a specific 40-character Bridge revision.
This is useful when preparing the ready program before updating the SDK's exact
Bridge pin.
